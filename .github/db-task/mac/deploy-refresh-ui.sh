...
const CSS='<style>/*DEALS-REFRESH-v5*/'
 ...
 +'#lastupd{margin-left:auto!important;font-size:clamp(13px,3vw,18px)!important;font-weight:800!important;line-height:1.25!important;color:#0d47a1!important}'
 // the round refresh button — same top-right spot as the containers page, dressed up
 +'#refreshbtn{position:fixed;top:14px;right:14px;z-index:9999;width:48px;height:48px;border-radius:50%;'
 +'background:linear-gradient(135deg,#1565d8,#0d47a1);color:#fff;border:none;font-size:22px;font-weight:800;'
 +'cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;line-height:1;'
 +'box-shadow:0 4px 14px rgba(13,40,80,.45);transition:transform .12s}'
 ...
 +'#rtoast{position:fixed;top:70px;right:14px;z-index:9999;background:rgba(9,25,55,.94);color:#fff;'
...
const JS='<script>/*DEALS-REFRESH-v5*/\n'
 +"(function(){\n"
 ...
 +"function setDate(ts){if(lu)lu.textContent=fmt(ts);}\n"
...
if(s!==before){fs.writeFileSync('index.html.bak-v5',before);fs.writeFileSync(F,s);console.log('v5 refresh UI: applied');}
else console.log('v5 refresh UI: NO CHANGE (unexpected)');
console.log('v5 present: '+((s.match(/DEALS-REFRESH-v5/g)||[]).length)+' (expect 2)');
NODE
...
console.log('v5 present: '+(/DEALS-REFRESH-v5/.test(s)?'yes':'NO — rerun deploy-refresh-ui.sh'));
P2EOF
echo "agent patcher updated (cleanup-only, keeps v5)" >> "$LOG"
...
echo "LIVE v5 blocks: $(curl -s https://classiccarsforsale-co.web.app/ | grep -c 'DEALS-REFRESH-v5')" >> "$LOG"

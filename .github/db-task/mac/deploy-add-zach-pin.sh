#!/bin/bash
# One-shot (run on the Mac): add Zach's full co-editor PIN (223344) to the
# dashboard and deploy. Idempotent - the edit is marker-fenced, so re-running
# changes nothing. Keeps a .bak-zachpin alongside index.html, and posts its
# log back to Claude.
set +e
...
# 1) the patch
# ---------------------------------------------------------------------------
curl -sL -o /tmp/patch-add-zach-pin.js "$RAWB/patch-add-zach-pin.js"
echo "patcher: $(wc -c < /tmp/patch-add-zach-pin.js) bytes" >> "$LOG"
node /tmp/patch-add-zach-pin.js "$IDX" >> "$LOG" 2>&1
PATCH_RC=$?
echo "patch exit: $PATCH_RC" >> "$LOG"

if [ "$PATCH_RC" != "0" ]; then
  echo "PATCH REPORTED A FAILURE — restoring and stopping, nothing deployed" >> "$LOG"
  [ -f "$IDX.bak-zachpin" ] && cp "$IDX.bak-zachpin" "$IDX"
  node -e 'const fs=require("fs");fetch("'"$DB"'/_debug/diag.json",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(fs.readFileSync("/tmp/zachpin.txt","utf8").slice(-5000))}).then(()=>console.log("LOG SENT")).catch(function(){})'
...
for MARK in 'ZACHPIN01' 'ZACHPIN02' 'ZACHPIN03' "ZACH_PIN = '223344'"; do
  echo "live has $MARK : $(grep -c "$MARK" /tmp/live.html)" >> "$LOG"
done

node -e 'const fs=require("fs");fetch("'"$DB"'/_debug/diag.json",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(fs.readFileSync("/tmp/zachpin.txt","utf8").slice(-5000))}).then(()=>console.log("LOG SENT — tell Claude done")).catch(e=>console.log("send failed "+e.message))'
echo "==== done ===="; cat "$LOG"
